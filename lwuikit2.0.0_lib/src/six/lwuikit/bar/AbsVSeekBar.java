...
public class AbsVSeekBar extends VProgressBar{
    ...
    public AbsVSeekBar(Context context) {
    	this(context,null,0);
    }

    public AbsVSeekBar(Context context, AttributeSet attrs) {
        this(context,attrs,0);
    }

    public AbsVSeekBar(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
		TypedArray a = context.obtainStyledAttributes(attrs, RE.styleable_SeekBar);
		if(null != a){
			int index = RE.get(RE.ATTR_ANDROID_THUMB);
			if(index >= 0){
				Drawable thumb = a.getDrawable(index);
		        setThumb(thumb);
			}
			index = RE.get(RE.ATTR_ANDROID_THUMBOFFSET);
			if(index >= 0){
				int thumbOffset = a.getDimensionPixelOffset(index, getThumbOffset());
		        setThumbOffset(thumbOffset);
			}
			index = RE.get(RE.ATTR_ANDROID_DISABLEDALPHA);
			if(index >= 0){
				mDisabledAlpha = a.getFloat(index, 0.5f);
			}
			a.recycle();
		}
    }
