...
public class EcoGallery extends EcoGalleryAbsSpinner implements GestureDetector.OnGestureListener {
	...
	public EcoGallery(Context context, AttributeSet attrs, int defStyle) {
		...
		init(context);
		TypedArray a = context.obtainStyledAttributes(attrs, RE.styleable_EcoGallery);
		if(null != a){
			int index = RE.get(RE.ATTR_GRAVITYS);
			if(index >= 0){
				int gravity = a.getInt(index, -1);
				if (gravity >= 0) {
					setGravity(gravity);
				}
			}
			index = RE.get(RE.ATTR_ANIMATIONDURATION);
			if(index >= 0){
				int animationDuration = a.getInt(index, 500);
				if (animationDuration > 0) {
					setAnimationDuration(animationDuration);
				}
			}
			index = RE.get(RE.ATTR_SPACING);
			if(index >= 0){
				int spacing = a.getDimensionPixelOffset(index, 0);
				setSpacing(spacing);
			}
			index = RE.get(RE.ATTR_UNSELECTEDALPHA);
			if(index >= 0){
				float unselectedAlpha = a.getFloat(index, 0.5f);
