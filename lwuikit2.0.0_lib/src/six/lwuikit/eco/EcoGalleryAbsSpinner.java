...
public abstract class EcoGalleryAbsSpinner extends EcoGalleryAdapterView<SpinnerAdapter> {
    ...
    public EcoGalleryAbsSpinner(Context context, AttributeSet attrs, int defStyle) {
        ...
        initAbsSpinner();
        TypedArray a = context.obtainStyledAttributes(attrs, RE.styleable_CustomAbsSpinner);
		if(null != a){
			int index = RE.get(RE.ATTR_ENTRIES);
			if(index >= 0){
				CharSequence[] entries = a.getTextArray(index);
		        if (entries != null) {
