...
public class H2PUtil {
    ...
    private static H2PUtil sInstance;
    ...
    protected H2PUtil(boolean hasChinaCollator) {
        mHasChinaCollator = hasChinaCollator;
    }

    public static H2PUtil getInstance() {
        synchronized(H2PUtil.class) {
            if (sInstance != null) {
                return sInstance;
            }
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA)) {
                    sInstance = new H2PUtil(true);
                    return sInstance;
                }
            }
            Log.w(TAG, "There is no Chinese collator, HanziToPinyin is disabled");
            sInstance = new H2PUtil(false);
            return sInstance;
        }
    }
    public String hanzi2pinyin(String displayName) {

		ArrayList<Token> tokens = get(displayName);
